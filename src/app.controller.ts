...
export class AppController {
  ...
  @HttpCode(200)
  @UseInterceptors(
    FileInterceptor('audio', { limits: { fileSize: 15 * 1024 * 1024 } }),
  )
  async voiceParse(
    @UploadedFile() file: Express.Multer.File | undefined,
    @Req() req: EdgeRequest,
  ) {
    return this.handleVoice(file, req);
  }
  ...
  private async handleVoice(
    ...
    const requestId = req._edgeRequestId ?? '';
    if (!file?.buffer?.length) {
      return edgeError('Missing multipart field "audio"', t0, requestId);
    }
    ...
    if (!transcript) {
      return edgeSuccess(
        { transcript: '', intent: 'unknown' as const },
        t0,
        requestId,
      );
    }

    const extracted = await this.openAi.parseVoiceTranscript(transcript);
    return edgeSuccess(extracted, t0, requestId);
  }
}
